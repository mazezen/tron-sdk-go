...
// https://github.com/fbsobreira/gotron-sdk/blob/master/pkg/abi/abi.go

type Param map[string]interface{}

// LoadFromJson json into ABI data
func LoadFromJson(jsonStr string) ([]Param, error) {
	if len(jsonStr) == 0 {
		return nil, nil
	}
	data := []Param{}
...
func convertToAddress(v interface{}) (common.Address, error) {
	...
	return common.Address{}, fmt.Errorf("invalid address: %T", v)
}
...
func convertToBytes(ty abi.Type, v interface{}) (interface{}, error) {
	// string
...
func GetParser(ABI *tronpb.SmartContract_ABI, method string) (abi.Arguments, error) {
	arguments := abi.Arguments{}
	for _, entry := range ABI.Entrys {
		if entry.Name == method {
			for _, out := range entry.Outputs {
				ty, err := abi.NewType(out.Type, "", nil)
				if err != nil {
...
func GetInputsParser(ABI *tronpb.SmartContract_ABI, method string) (abi.Arguments, error) {
	arguments := abi.Arguments{}
	for _, entry := range ABI.Entrys {
		if entry.Name == method {
