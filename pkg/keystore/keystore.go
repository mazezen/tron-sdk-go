...
func (ks *KeyStore) SignTx(a Account, tx *tronpb.Transaction) (*tronpb.Transaction, error) {
	// Look up the key to sign with and abort if it cannot be found
	ks.mu.RLock()
	defer ks.mu.RUnlock()
...
func (ks *KeyStore) SignTxWithPassphrase(a Account, passphrase string, tx *tronpb.Transaction) (*tronpb.Transaction, error) {
	_, key, err := ks.GetDecryptedKey(a, passphrase)
	if err != nil {
		return nil, err
	}
