...
func (w *keystoreWallet) SignTx(acc Account, tx *tronpb.Transaction) (*tronpb.Transaction, error) {
	// Make sure the requested account is contained within
	if !w.Contains(acc) {
		return nil, ErrUnknownAccount
	}
...
func (w *keystoreWallet) SignTxWithPassphrase(acc Account, passphrase string, tx *tronpb.Transaction) (*tronpb.Transaction, error) {
	// Make sure the requested account is contained within
	if !w.Contains(acc) {
		return nil, ErrUnknownAccount
	}
